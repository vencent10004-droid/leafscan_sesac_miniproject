...
|:---:|---|:---:|:---:|:---:|:---:|:---:|
| 🥇 1 | **resnet18** | **3.0e-03** | **0.745** | **0.817** | **0.741** | **12분** |
| 2 | convnext_tiny | 1.0e-03 | 계산 중* | 0.801 | — | 2.5h+ |
...
\* convnext_tiny는 epoch 6 시점 잠정 확정 (best epoch 3, 이후 미개선·과적합) — macro-F1은 run 종료 후 자동 산출.
...
| convnext 대비 resnet18 속도 우위 | **12배** |
...
공통: 입력 해상도 **224** · pretrained **체크** · freeze **3 epoch** · Optimizer **AdamW** · class weight **auto** · head 가중치 **crop 0.4 / stage 0.6** · Epoch **20** / Early stop **5회** · seed **42**.
아래 표에는 모델별로 **다르게 설정할 항목**과 실측 결과만 적는다.

### 🥇 1위 · resnet18

| 항목 | 설정값 |
|---|---|
| Learning rate | **3.0e-03** (슬라이더 **-2.5**) ★ 기본에서 바꿀 유일한 값 |
| 배치 | 16 (32 가능) |

| LR 탐색 (val_loss) | 1e-3 → 0.3813 | **3e-3 → 0.3521 ✔** | 1e-2 → 0.5157 |
|---|---|---|---|

* 결과: macro-F1 **0.745** · stage 정확도 0.817 · 정식기 recall 0.741 · **12분** (best epoch 2, 7에서 종료)
...
| 항목 | 설정값 |
|---|---|
| Learning rate | 1.0e-03 (슬라이더 -3.0, 기본) |
| 배치 | 16 고정 (4GB) |

| LR 탐색 (val_loss) | 1e-4 → 0.3667 | 3e-4 → 0.3231 | **1e-3 → 0.2993 ✔** |
|---|---|---|---|

* 결과(잠정): best val_loss **0.299 (전체 1위)** · stage 정확도 0.801 · **2.5시간+** (best epoch 3)
* 평가: val_loss는 최저지만 stage 정확도는 2위, 시간이 12배 이상 — 실용성 낮음
...
| 항목 | 설정값 |
|---|---|
| Learning rate | **3.0e-04** (슬라이더 **-3.5**) — 낮춰야 하는 모델 |
| 배치 | 16 고정 (32는 OOM 위험) |
...
>
> macro-F1 **0.745** (1위) · 학습 **12분** (반복 실험 최적) · crop 정확도 **1.000**
